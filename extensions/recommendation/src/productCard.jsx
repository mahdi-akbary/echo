...
export function ProductCard({ product, baseUrl }) {
  ...
  const [activeVariant, setActiveVariant] = useState(
    product?.variants?.edges.find((variant) => variant.node.availableForSale)
  );
  ...
  return (
    ...
    >
      ...
      <TextBlock>
        ...
        {!includePrice && (
          <Text size={button_size} appearance="subdued">
            {" "}
      ...
      <View inlineAlignment="end">
        ...
        >
          ...
          >
            <View>
              <Text size={button_size}>{addToCartLabel}</Text>
            </View>
            {includePrice && (
              <>
                <TextBlock> &#x2022; </TextBlock>
                <Text size={button_size}>{firstVariantPrice(product)}</Text>
              </>
            )}
          </InlineStack>
        </Button>
